...
function updateHistogramIncome(nameType){
	...
	var medianValues;
	if(nameType=="surname"){
		meanValues = state.surnameIncomeData.meanIncome;
		medianValues = state.surnameIncomeData.medianIncome;
	}
	else if(nameType == "forename"){
		meanValues = state.forenameIncomeData.meanIncome;
		medianValues = state.forenameIncomeData.medianIncome;
	}
...
function generateIncomeDistributionBar(numBins, nameType, incomeType){
	...
	bar.append("rect")
		...
		.append("title")
		.text(function(d){
			return "Range $" + formatCount(d.x) + " to $" + formatCount(d.x+binDif) + ":\n" +
