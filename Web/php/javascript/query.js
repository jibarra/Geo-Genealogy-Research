...
function updateKDEMap(data, nameType) {
	if(nameType == "surname"){
		...
		// refresh
		if (kde2.overlay != null) {
			kde2.overlay.setMap(null);
			delete kde2.overlay;
		}
		kde2.overlay = new KDEOverlay(kde2.map, "forename");
	...
	$.ajax(scriptLocation, {
		...
		success : function(data, textStatus, jqXHR) {
			...
			if(nameType=="surname"){
				...
				center = kde2.map.getBounds().getCenter();
				$('#loadingforename').hide();
...
function combineMapsCapture(){
	...
	var boundsOnEarth2 = kde2.map.getBounds()
	...
	if((latsw1 != latsw2) || (lngsw1 != lngsw2) || (latne1 != latne2) || (lngne1 != lngne2))
		return;
	if(kde.map.getZoom() != kde2.map.getZoom())
		return;
