...
@Path("/querySurname")
public class QuerySurname extends WFQuery{
...
//			sql = "select latitude, longitude from phonebook where surname=? and latitude between ? and ? and longitude between ? and ?";
			...
			sql = "select latitude, longitude from phonebook where surname=? and latitude between ? and ? and longitude between ? and ? LIMIT 25000";
			// 50,000 limit ~10-20 seconds
//			sql = "select latitude, longitude from phonebook where surname=? and latitude between ? and ? and longitude between ? and ? LIMIT 50000";
			// 100,000 limit ~30-45 seconds at max
			sql = "select latitude, longitude from phonebook where surname=? and latitude between ? and ? and longitude between ? and ? LIMIT 100000";
		...
		painter.drawKDEMap();
		queryIncome(surname, result);
		return new JSONWithPadding(result, callback);
	}
	
	public void queryIncome(String surname, QueryBean result){
		Connection connection = null;
		String sql;
		PreparedStatement statement = null;
		ResultSet resultset = null;
		...
		try {
			connection = connectDatabase();
			sql = "select average_mean, average_median from average_income_surname where surname=?";
			statement = connection.prepareStatement(sql, java.sql.ResultSet.TYPE_FORWARD_ONLY,
					java.sql.ResultSet.CONCUR_READ_ONLY);
